...
- `recurse-polling.md` - Polling for async operations
- `fixture-architecture.md` - Combining utilities with mergeTests
- `log-utils.md` - Logging API requests
...
```typescript
await expect(apiRequest({ method: 'GET', path: '/api/unstable' })).rejects.toThrow();
// Retries happen automatically, then final error caught
```
